...
function ProgressBar() {
  ...
  return (
    <motion.div
      ...
      style={{ scaleX, background: "linear-gradient(90deg,#f97316,#fb923c,#fdba74,#f97316)", backgroundSize: "200% 100%" }}
    />
  );
}
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ background: `radial-gradient(circle at ${glareX}% ${glareY}%, rgba(255,255,255,0.18) 0%, transparent 60%)` }}
      />
      {children}
    </motion.div>
  );
...
// Valores — diseño renovado con gradientes de fondo únicos por valor
const valores = [
  {
    label: "Amor",
...
export default function ConocenosClient() {
  ...
  // Cursor hover state lifted — PawCursor vive en el layout global,
  // aquí solo manejamos el estado local de hover para interacciones propias
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {valores.map((v, i) => (
              ...
              >
                ...
                <div className={`absolute -top-8 -right-8 w-32 h-32 rounded-full bg-gradient-to-br ${v.gradient} opacity-10 group-hover:opacity-20 group-hover:scale-110 transition-all duration-500`} />
                <div className={`absolute -bottom-10 -left-6 w-24 h-24 rounded-full bg-gradient-to-br ${v.gradient} opacity-5 group-hover:opacity-15 transition-all duration-700`} />
                ...
                <motion.div
                  className={`h-0.5 bg-gradient-to-r ${v.gradient} rounded-full mt-6 origin-left`}
                  initial={{ scaleX: 0 }}
      ...
      <section className="py-32 bg-slate-950 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="relative">
            <div className="hidden md:block absolute top-[16px] left-[12.5%] right-[12.5%] h-[2px] bg-white/10 z-0" />
            <motion.div className="hidden md:block absolute top-[16px] left-[12.5%] h-[2px] bg-gradient-to-r from-cyan-400 via-orange-500 to-pink-500 origin-left z-10 shadow-[0_0_20px_rgba(249,115,22,.8)]"
                ...
                <motion.div key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }}
                  ...
                  <div className="absolute -top-[54px] left-1/2 -translate-x-1/2 z-50">
                    <motion.div whileHover={{ scale: 1.2 }}
                      className={`w-[34px] h-[34px] rounded-full bg-gradient-to-br ${m.color} flex items-center justify-center border-4 border-slate-950 shadow-[0_0_30px_rgba(255,255,255,.4)]`}>
                      <div className="w-2.5 h-2.5 bg-white rounded-full" />
                    </motion.div>
                  </div>
                  <div className="relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-[#0b1020]/90 backdrop-blur-xl px-8 py-12 min-h-[430px] text-center flex flex-col justify-center items-center shadow-[0_0_40px_rgba(0,0,0,0.45)] transition-all duration-500 group-hover:-translate-y-2 group-hover:border-cyan-400/30">
                    <div className="absolute inset-0 opacity-40 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.12),transparent_55%)]" />
                    <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${m.color} flex items-center justify-center shadow-[0_0_30px_rgba(56,189,248,.6)] mb-5`}>
                    ...
                    <p className="text-white/60 text-sm leading-8 max-w-[260px]">{m.desc}</p>
